        ...
        match export_wallet_to_extension_format(keystore, &wallet_id, &password) {
            Ok(backup) => {
                ...
                match write_backup_to_file(&backup, &output_path) {
                    ...
                    Err(e) => {
                        app_state.log.push(format!("Failed to write backup file: {}", e));
                    }
                }
            }
            Err(e) => {
                app_state.log.push(format!("Failed to export wallet: {}", e));
            }
        }
    } else {
    ...
    match read_backup_from_file(&backup_path) {
        Ok(backup) => {
            ...
            match result {
                Ok(wallet_ids) => {
                    app_state.log.push(format!(
                        "Successfully imported {} wallet(s) from extension backup",
                        wallet_ids.len()
                    ));
                    for wallet_id in wallet_ids {
                        app_state.log.push(format!("  - Wallet ID: {}", wallet_id));
                    }
                }
                Err(e) => {
                    app_state.log.push(format!("Failed to import wallet: {}", e));
                }
            }
        }
        Err(e) => {
            app_state.log.push(format!("Failed to read backup file: {}", e));
        }
    }
}
...
pub async fn handle_convert_dkg_to_extension<C: frost_core::Ciphersuite + Send + Sync + 'static>(
    ...
    if !matches!(app_state.dkg_state, crate::utils::state::DkgState::Complete) {
        app_state.log.push("DKG process is not complete. Cannot export yet.".to_string());
        return;
    }
    ...
    if app_state.key_package.is_none() || app_state.group_public_key.is_none() || app_state.session.is_none() {
        app_state.log.push("Missing DKG results. Cannot export.".to_string());
        return;
    }
    ...
    match encrypt_for_extension(&extension_data, &password, &session.session_id) {
        Ok(encrypted) => {
            ...
            match write_backup_to_file(&backup, &output_path) {
                Ok(_) => {
                    app_state.log.push(format!(
                        "Successfully exported DKG session to extension format at: {}",
                        output_path
                    ));
                }
                Err(e) => {
        ...
        Err(e) => {
            app_state.log.push(format!("Failed to encrypt wallet data: {}", e));
        }
    }
}
