...
impl Keystore {
    ...
    pub fn new(base_path: impl AsRef<Path>, device_name: &str) -> Result<Self> {
        ...
        let index = if index_path.exists() {
            ...
            serde_json::from_reader(index_file).map_err(|e| KeystoreError::General(e.to_string()))?
        } else {
            ...
            serde_json::to_writer_pretty(index_file, &index).map_err(|e| KeystoreError::General(e.to_string()))?;
    ...
    fn save_index(&self) -> Result<()> {
        ...
        let index_file = File::create(index_path)?;
        serde_json::to_writer_pretty(index_file, &self.index).map_err(|e| KeystoreError::General(e.to_string()))?;
        Ok(())
    }
